...
* `Configuration means defining settings that tell the application how it should run, like server settings, database configuration, and other custom properties.`
...
*   **Modern apps →** `JAR + Docker + Kubernetes`<br>
    **WAR →** `mostly legacy projects or enterprise constraints`
...
* **POJO is a Java class that:**
    ...
    - And it does not depend on any framework-specific classes or interfaces
...
*   **We can configure various aspects of a Spring Boot application using this file:**
    ...
    2. **Database configuration:** `Configure datasource properties for different databases (e.g., MySQL, H2, PostgreSQL).`
        ``` properties
